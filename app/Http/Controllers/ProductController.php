...
class ProductController extends Controller
{
    ...
    public function approveInventory(Request $request,$inventoryId)
    {
		...
		$price = ProductPrice::where('inventoryId', $inventoryId)->first();
        ...
        }else if($price->mrp==''||$price->sprice==''||$price->mrp<0||$price->sprice<0){
			return back()->withErrors(['Please set price.']);
		}else {
            return back()->withErrors(['Inventory not found.']);
        }
    }
	public function checkInventory($id){
		$inventory = ProductInventory::with(['inventory_price'])->where('product_id',$id)->get();
		return $inventory;
	}
	public function inventory_update(Request $request){
		foreach($request->pr_id as $key=>$value){
			ProductPrice::where('id', $value)->update(['qty'=>$request->pr_qty[$key]]);
		}
