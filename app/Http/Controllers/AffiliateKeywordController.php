...
class AffiliateKeywordController extends Controller
{
    ...
    public function index() {
        ...
        $affiliateKeywordList = AffiliateKeyword ::where('status', '<>', 2)->get();
        return view('supperAdmin/affiliateKeyword/index',compact('affiliateKeywordList'));
    }
	
	 public function reviewkey()
    {

        $affiliateKeywordList = AffiliateKeyword::where('status', 2)->get();
        return view('supperAdmin/affiliateKeyword/reviewkey', compact('affiliateKeywordList'));
    }

    public function approved(Request $request, $id)
    {

       $qry = AffiliateKeyword::where('id', $id)->update(['status' => 1]);
        AccountAffiliateKeywords::where('keyword_id', $id)->update(['status' => 1]);
        return back()->with('message','Keyword Approved.');
    }
    ...
    public function destroy(AffiliateKeyword $affiliateKeyword) {
        ...
        if($result == 1) {
            ...
            return redirect()->back();//('/admin/affiliateKeyword');
