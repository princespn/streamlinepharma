...
<body class="animsition">
						    ...
						    <div class='col-lg-12'>
                                <div class="card">
									...
									<div class="card-body">
										...
										<?php if($user['plan_id']==0 || $user['plan_expiry']<date('Y-m-d H:i:s')){ ?>
										<div class="alert alert-error" role="alert" style='color:red;text-align:center;font-weight:bold;font-size:22px'>
											An active subscription is required before uploading files for analysis. Please <a href='my_profile.php'><u>Click here</u></a> to select a subscription plan.
										</div>
										<?php } ?>
										...
										<div class='mb-3 <?= ( $num_records && $data['store_ads'] ? "success-mb" : "error-mb" ) ?> store_ads' onclick="triggerFile('store_ads')">
											<p class="muted"> Upload Store Ad (Sales)</p>
											<div class="progress">
										...
										<div class='mb-3 <?= ( $num_records && $data['weekly_ads'] ? "success-mb" : "error-mb" ) ?> weekly_ads' onclick="triggerFile('weekly_ads')">
											<p class="muted"> Upload Weekly Ad (TPR)</p>
											<div class="progress">
										...
										<div class='mb-3 <?= ( $num_records && $data['department_lists'] ? "success-mb" : "error-mb" ) ?> department_lists' onclick="triggerFile('department_lists')">
											...
											<div style='text-align:center; font-family: "Courier Prime";'>											
												<input type='hidden' name='type' value='excel_upload'>
												<input type='button' style='font-size: 20px;' value='Initiate The Analysis' class='btn btn-secondary btn-xl' disabled='disabled' id='submit_btn'  onclick="this.disabled=true;window.location.replace('query.php?type=analyze');clickme()">
												<br>
												<div class="pppp"> 
    ...
    <script>
	function clickme(){
        var div =$("<div class='progress-barr'></div>");
       // $("body").append(div);
