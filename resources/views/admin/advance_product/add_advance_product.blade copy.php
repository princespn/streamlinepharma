...
<div class="row">
   <div class="col-12">
      <div class="card m-b-20">
		...
		<br>
		{!! Form::open(['url' => 'admin/insert_advance_product', 'class' => '']) !!}
		                     @if($data->grouping_name!=Null)
								 @foreach(json_decode($data->grouping_name,true)[0] as $kr=>$gr)
		                            <div class="form-check">
										<label class="form-check-label">
										  <input class="form-check-input" type="checkbox" @if(isset($pre_data)) @if($gr==$pre_data->grouping_name) checked @endif @endif  name='grouping_name[]' value='{{ $gr }}' >{{ json_decode($data->grouping_name,true)[1][$kr] }}{{ $pre_data->grouping_name }}
										</label>
									</div>
									@endforeach
									<br>
                              ...
                              @if($data->thumbnail_check)    
								...
								@if($data->product_tax_check)	
									...
									<div class="control-group">
										...
										<div class="controls">
											...
											<input type="number" name="product_tax" id="product_tax" class="form-control" value="@if(isset($pre_data)&&$pre_data->product_tax==$un) selected @endif"/>
										</div>
									</div>
								@endif	
								...
								@if($data->search_key_words_check)    
									<div class="control-group">
										<label class="control-label" for='txtInput'>Search Key Words</label>
											...
											<input type="text" class="form-control"    id="txtInput" data-role="tagsinput" name='search_key_words' @if(isset($pre_data)) value='{{ $pre_data->search_key_words }}' @endif />
								...
								@endif	
									...
									<div class="control-group">
										...
										<div class="controls">
											<select type="text" class="form-control"   name="is_affiliation" id="is_affiliation" onclick="if(this.value=='Yes'){$('.affiliation_div').removeClass('hide');}else{$('.affiliation_div').addClass('hide');}">
								    ...
								    <div class="control-group">
									   ...
									   @if(json_decode($data->additional_attribute)[1][$key]=='Checkboxes')
										...
										@else
											...
											<option  @if(isset($pre_data)&& (count($pre_data->json[0])==count(json_decode($data->additional_attribute)[0])) && $pre_data->json[2][$key] ==$opt )   checked @endif>{{ $opt }}</option>
										    @endforeach
										    </select>
										@endif
										</div>
