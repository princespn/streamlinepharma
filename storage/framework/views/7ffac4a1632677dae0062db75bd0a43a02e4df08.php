...
<div class="row">
    <div class="col-12">
        <div class="card m-b-20">
		    ...
		    <?php endif; ?>
			<?php echo Form::open(['url' => 'admin/dynamic_menu','class'=>'']); ?>
			  ...
			  <div class='form-group'>
				 ...
				 <select id="sub_category" name="sub_category" class="form-control">
				    <option></option>
					<?php $__currentLoopData = $sub_category; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
					   <option value='<?php echo e($row->id); ?>'><?php echo e($row->name); ?></option>
					<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
				 </select>
			  </div>
			  <div class='form-group'>
				 ...
				 <select id="setting" name="setting" class="form-control" required>
				    <option></option>
					<?php $__currentLoopData = $subscribed; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
					   <option value='<?php echo e($row->id); ?>'><?php echo e($row->name); ?></option>
					<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
				 </select>
			  </div>
				...
				<tbody>
			      <?php $__currentLoopData = $data; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key=>$row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
				     <tr>
					    ...
					    <td><?php echo e(($row->cat ? $row->cat->name : '')); ?></td>
					    <td><?php echo e(($row->sub_cat ? $row->sub_cat->name : '')); ?></td>
					    <td><?php echo e(($row->template ? $row->template->name : '')); ?></td>
					    <td>
						<?php if($row->template->grouping!=Null): ?>
					      <strong>Grouping Available on : </strong> <?php echo e($row->template->grouping); ?>
					    ...
					    <td>
						  <strong>Return : </strong><?php echo e($row->template->is_return); ?>

						  <br>
						  <strong>Replace : </strong><?php echo e($row->template->is_replace); ?>

						  <br>
						  <strong>Cancel Reason : </strong><?php echo ($row->template->cancel_reason ? '<br>'.implode('<br>',explode(',',$row->template->cancel_reason)) : ''); ?>

						  <br>
						  <a href="<?php echo e(url('admin/advance_product_category_action/'.$row->setting)); ?>" class='btn btn-primary btn-sm'>Edit</a>
						</td>
					    <td>
						<a href="<?php echo e(url('admin/add_advance_product/'.$row->id)); ?>" class='btn btn-primary btn-sm'>+</a>
						<a href="<?php echo e(url('admin/delete_dynamic_menu/'.$row->id)); ?>" class='btn btn-sm btn-xs btn-danger'>Delete</a>
					   ...
					   <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#grouping_modal" onclick="groupingFunction(<?php echo e($row->id); ?>,'<?php echo e($row->template->grouping); ?>')">
						  Grouping Name
					   </button>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/uniqueandcommon.com/public_html/globle_panel/public/new/resources/views/admin/advance_product/dynamic_menu.blade.php ENDPATH**/ ?>